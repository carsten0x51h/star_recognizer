...
{  
  ...
  */
  ...
  // So far only one filename at a time is supported
  if (vm.count("input-file")) {
	filename = vm["input-file"].as<std::string>();

	std::cout << "Filename specified: " 
			  << filename
			  << std::endl;
  } else {
  ...
  std::cout << "Applying star clustering..." << std::endl;
  clusterStars(binImg, & starInfos);

  
  std::cout << "Recognized " << starInfos.size() << " stars..." << std::endl;
 
  // Calc brightness boundaries for possible focusing stars
  float maxPossiblePixValue = pow(2.0, bitPix) - 1;
 
  // For each star
  std::cout << "Calculating measures for all stars..." << std::endl;
  ...
  for (StarInfoListT::iterator it = starInfos.begin(); it != starInfos.end(); ++it) {
	...
	std::cout << "cogCentroid=(" << setw(9) << std::get<0>(curStarInfo->cogCentroid)
			  << ", " << setw(9) << std::get<1>(curStarInfo->cogCentroid)
			  <<  "), " << setw(8) << ", maxPixValue: " << setw(8) << maxPixValue
			  << ", sat: " << curStarInfo->saturated << ", hfd: " << setw(10) << hfd
			  << ", fwhmHorz: " << setw(10) << fwhmHorz << ", fwhmVert: " << setw(10) << fwhmVert << std::endl;
  ...
  static const char* VISUALIZATION_FILENAME = "star_recognizer_out.jpeg";
  
  std::cout << "Illustration written to '" << VISUALIZATION_FILENAME << "'. Done." << std::endl; 
  rgbResized.save(VISUALIZATION_FILENAME);
